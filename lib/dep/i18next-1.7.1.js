...
var root = this, $ = root.jQuery || root.Zepto, i18n = {}, resStore = {}, currentLng, replacementCounter = 0, languages = [], initialized = false;
...
function _translate(potentialKeys, options) {
	...
	var notFound = _getDefaultValue(key, options), found = _find(key, options), lngs = options.lng ? f.toLanguages(options.lng) : languages, ns = options.ns || o.ns.defaultNs, parts;
	// split ns and key
	if (( typeof key === "string") && (key.indexOf(o.nsseparator) > -1)) {
	...
	if (notFound && notFound.indexOf(o.nsseparator) > -1) {
...
function _find(key, options) {
	...
	var found;
	var keys = key ? key.split(o.keyseparator) : [];
